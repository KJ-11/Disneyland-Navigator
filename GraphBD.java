...
public class GraphBD<NodeType, EdgeType extends Number> implements GraphADT<NodeType, EdgeType> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            // copies distance and extends by new edge
            this.distance = copyPath.distance + extendBy.weight.doubleValue();

            // instantiates copy of data sequence
            this.dataSequence = new LinkedList<>();

            // deep copies linked list
            if (copyPath.dataSequence != null) {
                this.dataSequence.addAll(copyPath.dataSequence);
            }

            // adds the target vertex to the copied data sequence
            this.dataSequence.add(extendBy.target.data);

            // make end our new target
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        // throw exceptions is vertices or any vertex data is null
        if (start == null || end == null) {
            throw new NoSuchElementException("Null vertices.");
        }
        if (vertices.get(start) == null || vertices.get(end) == null) {
            throw new NoSuchElementException("Vertices do not exist");
        }

        // initialize priority queue of paths
        PriorityQueue<Path> q = new PriorityQueue<>();

        // initialize list of visited vertices
        List<Vertex> visited = new LinkedList<>();

        // initialize path with just start node
        Path startPath = new Path(vertices.get(start));

        // adds the start node to visited and priority queue
        visited.add(startPath.start);
        q.add(startPath);

        while (!q.isEmpty()) {
            // pull path with the lowest weight and add the end node to visited
            Path path = q.poll();
            visited.add(path.end);

            // if that node is the target, return this path
            if (path.end.data.equals(end)) {
                return path;

                // otherwise, if the edges leaving this new node have not already been visited, add the path(s) to
                // the priority queue and continue the while loop.
            } else {
                for (int i = 0; i < path.end.edgesLeaving.size(); i++) {
                    if (!visited.contains(path.end.edgesLeaving.get(i).target)) {
                        q.add(new Path(path, path.end.edgesLeaving.get(i)));
                    }
                }
            }
        }
        // If the code reaches this point, the given vertices do not connect and the path does not exist.
